...
function validateStep(step, formData, roleConfig) {
  ...
  if (step === 1) {
    // Required common identity + location fields
    const requiredCommon = COMMON_FIELDS.filter(
      f => f.required && f.group !== 'Documents'
    )
...
export default function RegistrationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // ── Step 1: Create Supabase Auth user (handles password hashing securely)
      ...
      // ── Step 3: Build role-specific data object
      const roleData = {}
      enrichedRole.specificFields?.forEach(f => {
        if (formData[f.name] !== undefined) roleData[f.name] = formData[f.name]
      })

      // ── Step 4: Generate blockchain fingerprint (SHA-256 simulation)
      ...
      // ── Step 5: Insert entity record linked to the auth user
      const { error: insertError } = await supabase.from('entities').insert({
        ...
        location_gps:         formData.location_gps         || null,
      ...
      // ── Step 6: Save document references
      ...
      <div ref={topRef} className="min-h-screen pt-16 flex flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col px-4 sm:px-8 lg:px-12 py-10 max-w-2xl mx-auto w-full lg:max-w-none lg:mx-0">
          ...
          <div className="flex items-center gap-3 mb-8 animate-fadeInUp">
            ...
            <div>
              <p className="text-xs font-display font-semibold text-textSecondary uppercase tracking-widest">
                {enrichedRole.profileType}
              </p>
              <h1 className="text-xl font-display font-bold text-textPrimary leading-tight">
                {enrichedRole.label} Registration
              </h1>
            </div>
            <div className="ml-auto">
              <span className="nexus-badge text-xs">{nexusId}</span>
            </div>
          </div>
          ...
          {/* Submit error */}
          {submitError && (
            <div className="mt-4 px-4 py-3 bg-red-50 border border-error/20 rounded-xl text-sm text-error font-body">
              {submitError}
            </div>
          )}
          ...
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-100">
            <button
              onClick={() =>
                currentStep > 1 ? setCurrentStep(s => s - 1) : navigate('/register')
              }
              className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-gray-200
            ...
            >
              ...
              {currentStep > 1 ? 'Back' : 'Change Role'}
            </button>

            {currentStep < 3 ? (
              <button
                onClick={handleNext}
                className="flex items-center gap-2 px-8 py-3 rounded-xl bg-primary text-white
                  font-display font-semibold text-sm hover:bg-primary-600 shadow-sm
              ...
              <button
                ...
                className="flex items-center gap-2 px-8 py-3 rounded-xl bg-accent text-white
                  font-display font-semibold text-sm hover:bg-accent-dark shadow-sm
                  hover:shadow-md hover:scale-105 active:scale-95 transition-all duration-200
        ...
        {/* ── RIGHT PANEL ── */}
        <div className="hidden lg:flex w-80 xl:w-96 flex-col bg-[#060f1e] px-8 py-12
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mb-8">
              <svg viewBox="0 0 40 40" fill="none" className="w-12 h-12 mb-3">
                ...
                <path d="M13 22 L17 18 L20 21 L24 15 L27 18" stroke="white" strokeWidth="2.5"
                  strokeLinecap="round" strokeLinejoin="round" fill="none" />
              </svg>
              <h2 className="font-display font-bold text-white text-lg leading-tight">
                NordRock<br />TraceChain
              </h2>
              ...
              {[
                ...
                <div key={item.step}
                  className={`flex gap-3 p-3 rounded-xl transition-all duration-300
                    ${currentStep === item.step ? 'bg-white/8 border border-white/10' : 'opacity-40'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs
                    font-display font-bold flex-shrink-0
                    ${currentStep > item.step  ? 'bg-accent text-white'
            ...
            <div className="mt-auto">
              ...
              <div className="space-y-3">
                {[
                  'Your Nexus ID is permanently recorded on the blockchain ledger',
                  'An Inspector will review your documents within 2–5 business days',
                  'You will receive an email with your verification status',
                ].map((item, i) => (
