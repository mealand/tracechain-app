...
import { LogOut, Users, UserPlus, ShieldCheck, Activity, Loader2, AlertCircle } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      const { data } = await supabase.from('entities').select('verification_status').not('role', 'in', '(admin,inspector)')
      if (data) {
        setStats({
          total: data.length,
  ...
  const handleCreateUser = async (e) => {
    e.preventDefault()
    if (!form.full_name || !form.email || !form.password) { setCreateError('Name, email and password are required.'); return }
    setCreating(true); setCreateError(''); setCreateSuccess('')
    const nexusId = generateNexusID(createRole)
    const fingerprint = await generateBlockchainFingerprint({ nexus_id: nexusId, role: createRole, email: form.email })
    const { error } = await supabase.from('entities').insert({
      nexus_id: nexusId, role: createRole, full_name: form.full_name,
      email: form.email.toLowerCase(), phone: form.phone || null,
      password_hash: form.password, verification_status: 'verified',
      blockchain_fingerprint: fingerprint,
    })
    setCreating(false)
    if (error) { setCreateError(error.message); return }
    setCreateSuccess(`${createRole.charAt(0).toUpperCase() + createRole.slice(1)} created successfully. Nexus ID: ${nexusId}`)
  ...
  return (
    <div className="min-h-screen bg-surface">
      <nav className="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-gray-100 shadow-sm flex items-center px-6 gap-4">
        ...
        <span className="px-2.5 py-1 bg-error/10 text-error text-xs font-display font-bold rounded-full uppercase tracking-wide">Admin</span>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-sm font-body text-textSecondary mt-0.5">Add a new Admin or Inspector account.</p>
            </div>
            <button onClick={() => setShowCreateForm(v => !v)}
              className="flex items-center gap-2 btn-primary text-sm py-2.5">
              <UserPlus className="w-4 h-4" /> {showCreateForm ? 'Cancel' : 'Create User'}
            </button>
          </div>

          {showCreateForm && (
            <form onSubmit={handleCreateUser} className="border-t border-gray-100 pt-5 space-y-4 animate-fadeInUp">
              {createError && (
                <div className="flex items-start gap-2 bg-red-50 border border-error/20 rounded-xl px-4 py-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="form-label">Full Name <span className="text-error text-xs">*</span></label>
                  <input type="text" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} className="form-input" placeholder="e.g. John Doe" />
                </div>
                <div>
                  <label className="form-label">Email Address <span className="text-error text-xs">*</span></label>
                  <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="form-input" placeholder="john@nordrock.com" />
                </div>
                <div>
                  <label className="form-label">Phone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} className="form-input" placeholder="+234 000 0000" />
                </div>
                <div className="sm:col-span-2">
                  <label className="form-label">Password <span className="text-error text-xs">*</span></label>
                  <input type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} className="form-input" placeholder="Minimum 8 characters" />
                </div>
              </div>
              <button type="submit" disabled={creating}
                className="flex items-center gap-2 btn-primary text-sm disabled:opacity-60 disabled:cursor-not-allowed">
                {creating ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating...</> : <><UserPlus className="w-4 h-4" /> Create {createRole.charAt(0).toUpperCase() + createRole.slice(1)}</>}
              </button>
            </form>
          )}
        </div>
